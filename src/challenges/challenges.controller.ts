...
export class ChallengesController {
  ...
  async criarDesafio(@Body() createChallengeDto: CreateChallengeDto) {
    const { players, requester } = createChallengeDto;
    const playerOne = await lastValueFrom(
      this.clientAdmBackend.send('get-players-by-id', players[0]._id),
    );
    const playerTwo = await lastValueFrom(
      this.clientAdmBackend.send('get-players-by-id', players[1]._id),
    );
    ...
    const requesterIsMatchPlayer: Player = createChallengeDto.players.find(
  ...
  @Get()
  async consultarDesafios(@Query('playerId') playerId: string): Promise<any> {
    if (playerId) {
      const player: Player = await lastValueFrom(
        this.clientAdmBackend.send('get-players-by-id', playerId),
      );
      if (!player) {
    ...
    return await lastValueFrom(
      this.clientChallenges.send('get-challenges', { playerId, id: '' }),
    );
  }
  ...
  @Delete('/:id')
  async deletarDesafio(@Param('id') id: string) {
    const challenge: Challenge = await lastValueFrom(
