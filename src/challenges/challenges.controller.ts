...
export class ChallengesController {
  ...
  async createChallenge(
    ...
    const playerOne = await lastValueFrom(this.clientAdmBackend.send('get-player-by-id', players[0]))
    const playerTwo = await lastValueFrom(this.clientAdmBackend.send('get-player-by-id', players[1]))
    ...
    const requesterIsMatchPlayer = createChallengeDto.players.find(player => player == createChallengeDto.requester)
  ...
  async getChallengeByPlayer(
    ...
    if (id) {
      const player: Player = await lastValueFrom(this.clientAdmBackend.send('get-player-by-id', id))
      if (!player) {
